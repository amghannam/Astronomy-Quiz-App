...
public class ThirteenthQuestion extends JFrame {
	 ...
	 * Create the frame.
	 */
	public ThirteenthQuestion() {
		setResizable(false);
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 450, 336);
		setLocationRelativeTo(null); 
		setTitle("Astronomy Quiz: Question 13");
		...
		pnlQuestion.setBorder(new TitledBorder(new LineBorder(new Color(0, 0, 0), 1, true), 
				"Question 13", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 128)));
		pnlQuestion.setBounds(10, 11, 424, 80);
		contentPane.add(pnlQuestion);
		pnlQuestion.setLayout(null);
		
		JLabel lblQuestionText = new JLabel("<html> What is the given name for planets that do not "
		...
		JRadioButton rdbtFirst = new JRadioButton("Ice planets");
		...
		JRadioButton rdbtSecond = new JRadioButton("Secluded planets");
		...
		JRadioButton rdbtThird = new JRadioButton("Exoplanets");
		...
		JRadioButton rdbtFourth = new JRadioButton("Gas giants");
		...
		JRadioButton rdbtFifth = new JRadioButton("Rogue planets");
		...
		btnNextQuestion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (rdbtFifth.isSelected()) {
					...
					IncorrectlyAnsweredList.update("13");
				}
				dispose();
				new FourteenthQuestion().setVisible(true);
			}
		});
